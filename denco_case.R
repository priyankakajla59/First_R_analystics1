...
df1 = aggregate(sales$revenue , by=list(sales$custname), FUN=sum)#stored data in dataframe
head(df1)
str(df1)
...
# Freqency --------
names(sales)
head(sort(table(sales$custname), decreasing=T),10)

#xtab
#
head(sort(xtabs(~ custname, sales), decreasing=T))
#as.vector(tail(sort(table(sales$custname),)))
#
#
library(dplyr)
...
sales %>% dplyr::group_by(partnum) %>% dplyr::summarise(n = n()) %>% dplyr::arrange(desc(n)) #how many times a perticular parnum bought


# which parts have highest Profit : partno - sum(profit)
names(sales)
df4a = aggregate(margin ~ partnum, data=sales, FUN=sum) #which partnum gives max margin
#(head(sort([df4a$margin), decreasing=T),10)]
